 ...
 */
	...
	public function beforeFilter()
	{
		...
		$this->Auth->allow('login','logout', 'change_status');//,'add'
	}
		...
		if(empty($requestArgs)){
			...
			$this->Paginator->settings = array(
	            'conditions'=>array(
	                'User.role !='=>'admin', 
	                'OR' => array(
	                                'User.name LIKE' => '%'.trim($requestArgs['User']['search']).'%', 
	...
	public function admin_add() {
		...
		if ($this->request->is('post')) {	
			...
			if ($this->User->save($myArray)) {
				  ...
				  $emailArray =array();
                  $mailContent            = file_get_contents(WWW_ROOT. "/templates/verificationEmail.html");
                  $emailArray['username'] =$myArray['User']['name'];
                  $emailArray['email']    =$myArray['User']['email'];
                  $emailArray['password'] =$myArray['User']['password']; 
                  //unset($registerData['pass']); 
                  $this->varificationMail($myArray['User']['email'], $mailContent,$emailArray);
				$this->Flash->success(__('The user has been saved.'));
 ...
 */
	public function admin_edit($id = null) {
		...
		$cities = $this->City->find('all',array('conditions'=>array('City.state_id'=>$userDetails['User']['state_id'])));

		
		$this->set(compact('states', 'cities','userDetails'));		
